	...
	var demo = function() {
		...
		function subTableInit(e) {
			$('<div/>').attr('id', 'child_data_ajax_' + e.data.RecordID).appendTo(e.detailCell).KTDatatable({
				...
				rows: {
					afterTemplate: function (row, data, index) {
						row.find('.delete-doctor').on('click', function () {
							swal.fire({
								text: "هـل أنـت متـأكد مـن حـذف هـذا الطبيب ؟ ",
								confirmButtonText: "نعــم, أمسح !",
								icon: "warning",
								confirmButtonClass: "btn font-weight-bold btn-danger",
								showCancelButton: true,
								...
								if (result.value) {
									...
									$.ajax({
										method: 'delete',
										headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
										url: '/dashboard/organization_doctors/' + data.id,
										error: function (err) {
											if (err.hasOwnProperty('responseJSON')) {
												if (err.responseJSON.hasOwnProperty('message')) {
													swal.fire({
				...
				columns: [
					{
						...
						template: function(row) {
							return '\
                        <div class="dropdown dropdown-inline">\
                            <a href="/dashboard/organization_doctors/' + row.id  + '" class="btn btn-sm btn-clean btn-icon mr-2" title="عـرض">\
                             \<i class="flaticon-eye"></i>\
                            </a>\
                        </div>\
                        <a href="/dashboard/organization_doctors/'+ row.id +'/edit" class="btn btn-sm btn-clean btn-icon mr-2" title="تعديل">\
                            <span class="svg-icon svg-icon-md">\
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">\
                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
                                        <rect x="0" y="0" width="24" height="24"/>\
                        ...
                        <a href="javascript:;" class="btn btn-sm btn-clean btn-icon delete-doctor" title="حذف">\
                            <span class="svg-icon svg-icon-md">\
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">\
                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
                                        <rect x="0" y="0" width="24" height="24"/>\
